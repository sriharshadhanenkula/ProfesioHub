...
function myPostedEventItems(event) {
  return (
    <div style={{ marginBottom: "10px" }}>
      <Accordion>
        <AccordionSummary
        ...
        <AccordionDetails>
          ...
          <TableContainer component={Paper}>
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
              ...
              <TableBody>
                {event.EventApplicantsEmails.map((applicantEmail) => (
                  <DisplayEachApplicant applicantEmail={applicantEmail} />
                ))}
              </TableBody>
            </Table>
...
export function displayEventsPosted(allEvents, email) {
  const postedEvents = allEvents.filter((event) => {
  ...
  return (
    ...
    >
      ...
      {postedEvents.map((event) => myPostedEventItems(event))}
    </Container>
  );
}
